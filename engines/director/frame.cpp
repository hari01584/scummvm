...
Frame::Frame(const Frame &frame) {
	...
	_mainChannels._actionId = frame._mainChannels._actionId;
	_mainChannels._transArea = frame._mainChannels._transArea;
	_mainChannels._transDuration = frame._mainChannels._transDuration;
	...
	_mainChannels._soundType2 = frame._mainChannels._soundType2;
	_mainChannels._skipFrameFlag = frame._mainChannels._skipFrameFlag;
	_mainChannels._blend = frame._mainChannels._blend;

	_mainChannels._colorTempo = frame._mainChannels._colorTempo;
	_mainChannels._colorSound1 = frame._mainChannels._colorSound1;
	_mainChannels._colorSound2 = frame._mainChannels._colorSound2;
	_mainChannels._colorScript = frame._mainChannels._colorScript;
	_mainChannels._colorTrans = frame._mainChannels._colorTrans;

	_mainChannels._palette = frame._mainChannels._palette;

	_score = frame._score;

	debugC(1, kDebugLoading, "Frame. action: %s transType: %d transDuration: %d", _mainChannels._actionId.asString().c_str(), _mainChannels._transType, _mainChannels._transDuration);
...
void Frame::readMainChannelsD2(Common::MemoryReadStreamEndian &stream, uint16 offset, uint16 size) {
	...
	while (stream.pos() < finishPosition) {
		...
		case 0: // Sound/Tempo/Transition
			_mainChannels._actionId = CastMemberID(stream.readByte(), DEFAULT_CAST_LIB);
			break;
		case 1:
			// type: 0x17 for sounds (sound is cast id), 0x16 for MIDI (sound is cmd id)
			_mainChannels._soundType1 = stream.readByte();
			break;
		case 2: {
				// 0x80 is whole stage (vs changed area), rest is duration in 1/4ths of a second
				uint8 transFlags = stream.readByte();
				if (transFlags & 0x80)
					_mainChannels._transArea = 1;
				else
					_mainChannels._transArea = 0;
				_mainChannels._transDuration = (transFlags & 0x7f) * 250; // Duration is in 1/4 secs
			}
			break;
		case 3:
			_mainChannels._transChunkSize = stream.readByte();
			break;
		case 4:
			_mainChannels._tempo = stream.readByte();
			break;
		case 5:
			_mainChannels._transType = static_cast<TransitionType>(stream.readByte());
			break;
		case 6:
			_mainChannels._sound1 = CastMemberID(stream.readUint16(), DEFAULT_CAST_LIB);
			break;
		case 8:
			_mainChannels._sound2 = CastMemberID(stream.readUint16(), DEFAULT_CAST_LIB);
			break;
		case 10:
			_mainChannels._soundType2 = stream.readByte();
			break;
		case 11:
			_mainChannels._skipFrameFlag = stream.readByte();
			break;
		case 12:
			_mainChannels._blend = stream.readByte();
			break;
		case 13:
			stream.read(unk, 1);
		...
		case 16: {
				...
				if (paletteId == 0) {
					_mainChannels._palette.paletteId = CastMemberID(0, 0);
				} else if (paletteId < 0) {
					_mainChannels._palette.paletteId = CastMemberID(paletteId, -1);
				} else {
					_mainChannels._palette.paletteId = CastMemberID(paletteId, DEFAULT_CAST_LIB);
				}
				if (!_mainChannels._palette.paletteId.isNull())
					_mainChannels._scoreCachedPaletteId = _mainChannels._palette.paletteId;
			}
			break;
		case 18:
			// loop points for color cycling
			_mainChannels._palette.firstColor = g_director->transformColor(stream.readByte() ^ 0x80);
			_mainChannels._palette.lastColor = g_director->transformColor(stream.readByte() ^ 0x80);
			break;
		case 20:
			_mainChannels._palette.flags = stream.readByte();
			_mainChannels._palette.colorCycling = (_mainChannels._palette.flags & 0x80) != 0;
			_mainChannels._palette.normal = (_mainChannels._palette.flags & 0x60) == 0x00;
			_mainChannels._palette.fadeToBlack = (_mainChannels._palette.flags & 0x60) == 0x60;
			_mainChannels._palette.fadeToWhite = (_mainChannels._palette.flags & 0x60) == 0x40;
			_mainChannels._palette.autoReverse = (_mainChannels._palette.flags & 0x10) != 0;
			_mainChannels._palette.overTime = (_mainChannels._palette.flags & 0x04) != 0;
			_mainChannels._palette.speed = stream.readByte();
			break;
		case 22:
			_mainChannels._palette.frameCount = stream.readUint16();
			break;
		case 24:
			_mainChannels._palette.cycleCount = stream.readUint16();
			break;
		case 26:
			stream.read(unk, 6);
	...
	_mainChannels._transChunkSize = CLIP<byte>(_mainChannels._transChunkSize, 0, 128);
	_mainChannels._transDuration = CLIP<uint16>(_mainChannels._transDuration, 0, 32000);  // restrict to 32 secs
}
...
void Frame::readMainChannelsD4(Common::MemoryReadStreamEndian &stream, uint16 offset, uint16 size) {
	...
	while (stream.pos() < finishPosition) {
		...
		case 0:
			...
			break;
		case 1:
			_mainChannels._soundType1 = stream.readByte(); // type: 0x17 for sounds (sound is cast id), 0x16 for MIDI (sound is cmd id)
			break;
		case 2: {
				uint8 transFlags = stream.readByte();
		...
		case 12:
			...
			break;
		case 13:
			_mainChannels._colorTempo = stream.readByte();
			break;
		case 14:
			_mainChannels._colorSound1 = stream.readByte();
			break;
		case 15:
			_mainChannels._colorSound2 = stream.readByte();
			break;
		case 16:
			_mainChannels._actionId = CastMemberID(stream.readUint16(), DEFAULT_CAST_LIB);
			break;
		case 18:
			_mainChannels._colorScript = stream.readByte();
			break;
		case 19:
			_mainChannels._colorTrans = stream.readByte();
			break;
		case 20: {
				// palette, 13 bytes
				int16 paletteId = stream.readSint16();
		...
		case 22:
			// loop points for color cycling
			_mainChannels._palette.firstColor = g_director->transformColor(stream.readByte() + 0x80); // 22
			_mainChannels._palette.lastColor = g_director->transformColor(stream.readByte() + 0x80); // 23
			break;
		case 24:
			_mainChannels._palette.flags = stream.readByte(); // 24
			_mainChannels._palette.colorCycling = (_mainChannels._palette.flags & 0x80) != 0;
			_mainChannels._palette.normal = (_mainChannels._palette.flags & 0x60) == 0x00;
			_mainChannels._palette.fadeToBlack = (_mainChannels._palette.flags & 0x60) == 0x60;
			_mainChannels._palette.fadeToWhite = (_mainChannels._palette.flags & 0x60) == 0x40;
			_mainChannels._palette.autoReverse = (_mainChannels._palette.flags & 0x10) != 0;
			_mainChannels._palette.overTime = (_mainChannels._palette.flags & 0x04) != 0;
			_mainChannels._palette.speed = stream.readByte(); // 25
			break;
		case 26:
			_mainChannels._palette.frameCount = stream.readUint16(); // 26
			break;
		case 28:
			_mainChannels._palette.cycleCount = stream.readUint16(); // 28
			break;
		case 30:
			_mainChannels._palette.fade = stream.readByte(); // 30
			_mainChannels._palette.delay = stream.readByte(); // 31
			_mainChannels._palette.style = stream.readByte(); // 32
			break;
		case 33:
			unk1 = stream.readByte();
			if (unk1)
		...
		case 36:
			...
			break;
		case 38:
			_mainChannels._palette.colorCode = stream.readByte();
			break;
		case 39:
			unk1 = stream.readByte();
			if (unk1)
...
void Frame::readMainChannelsD5(Common::MemoryReadStreamEndian &stream, uint16 offset, uint16 size) {
	...
	while (stream.pos() < finishPosition) {
		...
		case 0: {
				...
				_mainChannels._actionId = CastMemberID(actionId, actionCastLib);
			}
			break;
		case 2: {
				uint16 actionId = stream.readUint16();
				_mainChannels._actionId = CastMemberID(actionId, _mainChannels._actionId.castLib);  // Inherit castLinb from previous frame
			}
			break;
		case 4: {
				uint16 sound1CastLib = stream.readUint16();
				uint16 sound1Id = stream.readUint16();
				_mainChannels._sound1 = CastMemberID(sound1Id, sound1CastLib);
			}
			break;
		case 6: {
				uint16 sound1Id = stream.readUint16();
				_mainChannels._sound1 = CastMemberID(sound1Id, _mainChannels._sound1.castLib);	// Inherit castLinb from previous frame
			}
			break;
		case 8: {
				uint16 sound2CastLib = stream.readUint16();
				uint16 sound2Id = stream.readUint16();
				_mainChannels._sound2 = CastMemberID(sound2Id, sound2CastLib);
			}
			break;
		case 10: {
				uint16 sound2Id = stream.readUint16();
				_mainChannels._sound2 = CastMemberID(sound2Id, _mainChannels._sound2.castLib);	// Inherit castLinb from previous frame
			}
			break;
		case 12: {
				uint16 transCastLib = stream.readUint16();
				uint16 transId = stream.readUint16();
				_mainChannels._trans = CastMemberID(transId, transCastLib);
			}
			break;
		case 16:
			stream.read(unk, 4);
		...
		case 24: {
				...
				_mainChannels._palette.paletteId = CastMemberID(paletteId, paletteCastLib);
				if (!_mainChannels._palette.paletteId.isNull())
					_mainChannels._scoreCachedPaletteId = _mainChannels._palette.paletteId;
			}
			break;
		case 28:
			_mainChannels._palette.speed = stream.readByte(); // 28
			_mainChannels._palette.flags = stream.readByte(); // 29
			_mainChannels._palette.colorCycling = (_mainChannels._palette.flags & 0x80) != 0;
			_mainChannels._palette.normal = (_mainChannels._palette.flags & 0x60) == 0x00;
			_mainChannels._palette.fadeToBlack = (_mainChannels._palette.flags & 0x60) == 0x60;
			_mainChannels._palette.fadeToWhite = (_mainChannels._palette.flags & 0x60) == 0x40;
			...
			break;
		case 30:
			_mainChannels._palette.firstColor = g_director->transformColor(stream.readByte() + 0x80); // 30
			_mainChannels._palette.lastColor = g_director->transformColor(stream.readByte() + 0x80); // 31
			break;
		case 32:
			_mainChannels._palette.frameCount = stream.readUint16(); // 32
			break;
		case 34:
			_mainChannels._palette.cycleCount = stream.readUint16(); // 34
			break;
		case 36: {
				stream.read(unk, 12);
...
Common::String Frame::formatChannelInfo() {
	...
	result += Common::String::format("TMPO:   tempo: %d, skipFrameFlag: %d, blend: %d\n",
		_mainChannels._tempo, _mainChannels._skipFrameFlag, _mainChannels._blend);
	if (_mainChannels._palette.paletteId.isNull()) {
		result += Common::String::format("PAL:    paletteId: %s, firstColor: %d, lastColor: %d, flags: %d, cycleCount: %d, speed: %d, frameCount: %d, fade: %d, delay: %d, style: %d\n",
			_mainChannels._palette.paletteId.asString().c_str(), _mainChannels._palette.firstColor, _mainChannels._palette.lastColor, _mainChannels._palette.flags,
			_mainChannels._palette.cycleCount, _mainChannels._palette.speed, _mainChannels._palette.frameCount,
			_mainChannels._palette.fade, _mainChannels._palette.delay, _mainChannels._palette.style);
	} else {
	...
	result += Common::String::format("TRAN:   transType: %d, transDuration: %d, transChunkSize: %d\n",
		_mainChannels._transType, _mainChannels._transDuration, _mainChannels._transChunkSize);
	result += Common::String::format("SND: 1  sound1: %d, soundType1: %d\n", _mainChannels._sound1.member, _mainChannels._soundType1);
	result += Common::String::format("SND: 2  sound2: %d, soundType2: %d\n", _mainChannels._sound2.member, _mainChannels._soundType2);
	result += Common::String::format("LSCR:   actionId: %d\n", _mainChannels._actionId.member);
