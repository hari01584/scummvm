...
void Score::update() {
	...
	if (!_window->_newMovieStarted && !_vm->_playbackPaused) {
		...
		if (_currentFrame->_mainChannels._transType == 0 && _currentFrame->_mainChannels._trans.isNull()) {
	...
	byte tempo = _currentFrame->_mainChannels._scoreCachedTempo;
	// puppetTempo is overridden by changes in score tempo
	if (_currentFrame->_mainChannels._tempo || tempo != _lastTempo) {
		_puppetTempo = 0;
...
bool Score::renderTransition(uint16 frameId) {
	...
	if (tp) {
		setLastPalette(frameId);
		_window->playTransition(frameId, tp->duration, tp->area, tp->chunkSize, tp->type, currentFrame->_mainChannels._scoreCachedPaletteId);
		delete _window->_puppetTransition;
		_window->_puppetTransition = nullptr;
		return true;
	} else if (currentFrame->_mainChannels._transType) {
		setLastPalette(frameId);
		_window->playTransition(frameId, currentFrame->_mainChannels._transDuration, currentFrame->_mainChannels._transArea, currentFrame->_mainChannels._transChunkSize, currentFrame->_mainChannels._transType, currentFrame->_mainChannels._scoreCachedPaletteId);
		return true;
	} else if (!currentFrame->_mainChannels._trans.isNull()) {
		CastMember *member = _movie->getCastMember(currentFrame->_mainChannels._trans);
		if (member && member->_type == kCastTransition) {
			TransitionCastMember *trans = static_cast<TransitionCastMember *>(member);
			setLastPalette(frameId);
			_window->playTransition(frameId, trans->_durationMillis, trans->_area, trans->_chunkSize, trans->_transType, currentFrame->_mainChannels._scoreCachedPaletteId);
			return true;
		}
	}
	return false;
...
bool Score::renderPrePaletteCycle(uint16 frameId, RenderMode mode) {
	...
	CastMemberID currentPalette = _currentFrame->_mainChannels._palette.paletteId;
	if (currentPalette.isNull())
		return false;

	if (!_currentFrame->_mainChannels._palette.colorCycling &&
		!_currentFrame->_mainChannels._palette.overTime) {

		int frameRate = CLIP<int>(_currentFrame->_mainChannels._palette.speed, 1, 30);
		...
		if (_currentFrame->_mainChannels._palette.normal) {
			// If the target palette ID is the same as the previous palette ID,
			// a normal fade is a no-op.
			if (_currentFrame->_mainChannels._palette.paletteId == g_director->_lastPalette) {
				return false;
			}
			...
			if (_currentFrame->_mainChannels._palette.fadeToBlack) {
			...
			} else if (_currentFrame->_mainChannels._palette.fadeToWhite) {
...
void Score::setLastPalette(uint16 frameId) {
	...
	if (currentPalette.isNull()) {
		...
		currentPalette = _currentFrame->_mainChannels._scoreCachedPaletteId;
	...
	if (paletteChanged) {
		...
		if (_currentFrame->_mainChannels._palette.colorCycling || isCachedPalette)
...
bool Score::isPaletteColorCycling() {
	return _currentFrame->_mainChannels._palette.colorCycling;
}
...
void Score::renderPaletteCycle(uint16 frameId, RenderMode mode) {
	...
	// Cycle speed in FPS
	int speed = _currentFrame->_mainChannels._palette.speed;
	if (speed == 0)
		return;
	...
	if (_currentFrame->_mainChannels._palette.colorCycling) {
		...
		int firstColor = _currentFrame->_mainChannels._palette.firstColor;
		int lastColor = _currentFrame->_mainChannels._palette.lastColor;

		if (_currentFrame->_mainChannels._palette.overTime) {
			...
			debugC(2, kDebugImages, "Score::renderPaletteCycle(): color cycle palette %s, from colors %d to %d, over %d steps %d times", currentPalette.asString().c_str(), firstColor, lastColor, steps, _currentFrame->_mainChannels._palette.cycleCount);
			for (int i = 0; i < _currentFrame->_mainChannels._palette.cycleCount; i++) {
				...
				if (_currentFrame->_mainChannels._palette.autoReverse) {
					for (int j = 0; j < steps; j++) {
						uint32 startTime = g_system->getMillis();
		...
		int frameCount = _currentFrame->_mainChannels._palette.frameCount;
		byte calcPal[768];
...
void Score::playSoundChannel(uint16 frameId, bool puppetOnly) {
	debugC(5, kDebugSound, "playSoundChannel(): Sound1 %s Sound2 %s", _currentFrame->_mainChannels._sound1.asString().c_str(), _currentFrame->_mainChannels._sound2.asString().c_str());
	DirectorSound *sound = _window->getSoundManager();

	if (sound->isChannelPuppet(1)) {
		sound->playPuppetSound(1);
	} else if (!puppetOnly) {
		if (_currentFrame->_mainChannels._soundType1 >= kMinSampledMenu && _currentFrame->_mainChannels._soundType1 <= kMaxSampledMenu) {
			sound->playExternalSound(_currentFrame->_mainChannels._soundType1, _currentFrame->_mainChannels._sound1.member, 1);
		} else {
			sound->playCastMember(_currentFrame->_mainChannels._sound1, 1);
		}
	}
	...
	if (sound->isChannelPuppet(2)) {
		...
		if (_currentFrame->_mainChannels._soundType2 >= kMinSampledMenu && _currentFrame->_mainChannels._soundType2 <= kMaxSampledMenu) {
			sound->playExternalSound(_currentFrame->_mainChannels._soundType2, _currentFrame->_mainChannels._sound2.member, 2);
		} else {
			sound->playCastMember(_currentFrame->_mainChannels._sound2, 2);
		}
	}
...
bool Score::readOneFrame() {
	...
	if (frameSize > 0) {
		...
		if (_currentFrame->_mainChannels._tempo && _currentFrame->_mainChannels._tempo <= 120)
			_currentTempo = _currentFrame->_mainChannels._tempo;
		_currentFrame->_mainChannels._scoreCachedTempo = _currentFrame->_mainChannels._tempo ? _currentFrame->_mainChannels._tempo : _currentTempo;
...
Common::String Score::formatChannelInfo() {
	...
	result += Common::String::format("TMPO:   tempo: %d, skipFrameFlag: %d, blend: %d, currentFPS: %d\n",
		frame._mainChannels._tempo, frame._mainChannels._skipFrameFlag, frame._mainChannels._blend, _currentFrameRate);
	if (!frame._mainChannels._palette.paletteId.isNull()) {
		result += Common::String::format("PAL:    paletteId: %s, firstColor: %d, lastColor: %d, flags: %d, cycleCount: %d, speed: %d, frameCount: %d, fade: %d, delay: %d, style: %d, currentId: %s, defaultId: %s\n",
			frame._mainChannels._palette.paletteId.asString().c_str(), frame._mainChannels._palette.firstColor, frame._mainChannels._palette.lastColor, frame._mainChannels._palette.flags,
			frame._mainChannels._palette.cycleCount, frame._mainChannels._palette.speed, frame._mainChannels._palette.frameCount,
			frame._mainChannels._palette.fade, frame._mainChannels._palette.delay, frame._mainChannels._palette.style, g_director->_lastPalette.asString().c_str(), defaultPalette.asString().c_str());
	} else {
	...
	result += Common::String::format("TRAN:   transType: %d, transDuration: %d, transChunkSize: %d\n",
		frame._mainChannels._transType, frame._mainChannels._transDuration, frame._mainChannels._transChunkSize);
	result += Common::String::format("SND: 1  sound1: %d, soundType1: %d\n", frame._mainChannels._sound1.member, frame._mainChannels._soundType1);
	result += Common::String::format("SND: 2  sound2: %d, soundType2: %d\n", frame._mainChannels._sound2.member, frame._mainChannels._soundType2);
	result += Common::String::format("LSCR:   actionId: %d\n", frame._mainChannels._actionId.member);
